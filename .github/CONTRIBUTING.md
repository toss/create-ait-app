...
스캐폴딩 테스트는 생성한 프로젝트를 실제로 설치하고 빌드해요. `dist/index.html`을 브라우저로 렌더링하고 `.ait` 산출물이 나오는지 확인해요.
