...
- `--inline`은 새 프로젝트 생성 시 프로젝트 경로, `--pm`, `--template` 또는 `--tds`가
  모두 있을 때만 실행해요. 필수값이 빠졌을 때 대화형 프롬프트로 넘어가지 말고 즉시
  오류를 내야 해요. init은 계약이 달라요 — 경로는 생략할 수 있고(기본 `.`), `--pm`만
  필수이며 `--template`·`--tds`·`--sample`은 금지해요.
- 공식 문서 본문을 저장소에 복사하지 말고 동적 문서 조회 절차만 유지해 주세요.
- init은 기존 파일을 덮어쓰지 않아요. 덮어써야만 하는 상황은 assertAdoptableProject가
  미리 거부해요.
- init은 실패해도 대상 디렉터리를 삭제하지 않아요.
